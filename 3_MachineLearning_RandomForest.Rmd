...
#Removed the recipe with 2031 user ratings, and NA's
...
plot(prediction_fullmodel, recipes_test$n_ratings, ylab =  "Recipe Popularity observed", xlab = "Predicted Recipe Popularity") #that's not very good
abline(0,1)
...
-  Model did not perform well in the test set. This is because as shown by the correlation matrix  the predictors were uncorrelated with recipe popularity `n_ratings`. Therefore, the random forest algorithm  is forced to choose amongst only "noise" variables at many of its splits leading to poor performance.
